...
export default function PayslipListScreen() {
    ...
    const calculateNetWage = (slipId: number) => {
        ...
        const netLine = slipLines.find(l => l.code === 'NET');
        if (netLine) return netLine.total;
        const gross = slipLines.find(l => l.code === 'GROSS')?.total ?? 0;
        const deductions = slipLines
            .filter(l => l.categoryId?.name === 'Deduction' || l.categoryId?.name === 'DED')
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <View className="w-full md:w-24 flex-row justify-end gap-2 border-t border-slate-100 md:border-t-0 pt-4 md:pt-0">
                                            ...
                                            <TouchableOpacity
                                                disabled
                                                className="p-2 rounded-lg opacity-40"
