...
export class AttendanceRepository {
    ...
    async getMonthAttendance(employeeId: number, year: number, month: number): Promise<AttendanceRecord[]> {
        const firstIso = new Date(Date.UTC(year, month - 1, 1)).toISOString().slice(0, 19).replace('T', ' ');
        const lastIso = new Date(Date.UTC(year, month, 1)).toISOString().slice(0, 19).replace('T', ' ');
    ...
    async getMyLeaveRequests(employeeId: number, filters: LeaveFilters = {}, limit = 20, offset = 0): Promise<LeaveRequest[]> {
        ...
        if (filters.year) {
            ...
            // Day 0 of the following month = last day of the target month (JS overflow)
            const endMonthDay = new Date(Date.UTC(filters.year, filters.month ?? 12, 0)).getUTCDate();
    ...
    async getTeamUpcomingLeaves(departmentId: number): Promise<LeaveRequest[]> {
        ...
        const nextWeekStr = new Date(Date.now() + 14 * 86_400_000).toISOString().slice(0, 10) + ' 23:59:59';
